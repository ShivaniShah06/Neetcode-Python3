...
class Solution:
    ...
    def longest_substring(self, s: str) -> int:
        ...
        while end < len(s): # Run the loop until the end pointer reaches the last index
            if s[end] in d and d[s[end]] >= start: # If the value at the end pointer in `s` exist in the dictionary and if the value for that letter in dictionary is greater than the value of the start pointer i.e. if the index of that letter in the dictionary is greater than the index at which the start pointer is pointing (that means that the letter is repeated!)
